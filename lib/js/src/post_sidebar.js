$(document).ready(function() {
    ...
    $tocSelector.on('activate.bs.scrollspy',
    function() {
    ...
    function removeCurrentActiveClass() {
        $(tocSelector + ' ' + activeCurrentSelector).removeClass(activeCurrentSelector.substring(1));
    }
    function processTOC() {
        getTOCMaxHeight();
    ...
    function getTOCMaxHeight() {
        var height = $('.sidebar').height() - $tocSelector.position().top - $('.post-toc-indicator-bottom').height();
        $tocSelector.css('height', height);
        return height;
    }
    function toggleTOCOverflowIndicators() {
        tocOverflowIndicator('.post-toc-indicator-top', $tocSelector.scrollTop() > 0 ? 'show': 'hide');
        tocOverflowIndicator('.post-toc-indicator-bottom', $tocSelector.scrollTop() >= $tocSelector.find('ol').height() - $tocSelector.height() ? 'hide': 'show')
    }
    ...
    function onScroll(element) {
        element.on('mousewheel DOMMouseScroll',
        function(event) {
    ...
    function tocOverflowIndicator(indicator, action) {
        ...
        $indicator.velocity ? $indicator.velocity('stop').velocity({
            opacity: opacity
        },
        {
            duration: 100
    ...
    for (i in menu) {
        ...
        if (h2.length) {
            ...
            for (j in h2) {
                jj++;
                h3 = h2[j];
                h3Node = $('<li class="nav-item nav-level-3"><a class="nav-link" href="#' + (h3.replace(/ /g, '')) + '"><span class="nav-number">' + (ii).toString() + '.' + (jj).toString() + '</span> <span class="nav-text">' + h3 + '</span></a></li>') h3List.append(h3Node);
            }
